...
async def run_offer(offer_peer):
    ...
    # logging ICE
    @offer_peer.on("icegatheringstatechange")
    def on_ice_gathering_state_change():
        print("ICE Gathering State:", offer_peer.iceGatheringState)

    @offer_peer.on("icecandidate")
    def on_ice_candidate(candidate):
        print("Novo ICE Candidate:", candidate)

    @offer_peer.on("iceconnectionstatechange")
    def on_ice_state_change():
        print("ICE Connection State of offer:", offer_peer.iceConnectionState)

    offer = await offer_peer.createOffer()  # create the SDP offer - IS CORRECT TO SAY 'SDP OFFER'?
...
async def run_answer(answer_peer, offer_peer_SDP):
    ...
    # logging ICE
    @answer_peer.on("icegatheringstatechange")
    def on_ice_gathering_state_change():
        print("ICE Gathering State:", answer_peer.iceGatheringState)

    @answer_peer.on("icecandidate")
    def on_ice_candidate(candidate):
        print("Novo ICE Candidate:", candidate)

    @answer_peer.on("iceconnectionstatechange")
    def on_ice_state_change():
        print("ICE Connection State of response:", answer_peer.iceConnectionState)

    await answer_peer.setRemoteDescription(offer_peer_SDP)  # receives the offer peer description and set its remote description as the offer description
...
async def main():
    ...
    offer_peer_SDP = await run_offer(offer_peer)
    answer_peer_SDP = await run_answer(answer_peer, offer_peer_SDP)

    await offer_peer.setRemoteDescription(answer_peer_SDP)
